...
esp_err_t ssd1306_attach(i2c_master_bus_handle_t bus, ssd1306_t *dev, uint8_t addr)
{
    ...
    if (ret == ESP_OK) {
        ...
        ESP_LOGE(TAG, "Failed to add new device. err: %s", esp_err_to_name(ret));
    }
    return ret;
}
...
esp_err_t ssd1306_write_stream(ssd1306_t *dev, const uint8_t *buffer, size_t size)
{
    ...
    i2c_master_transmit_multi_buffer_info_t buf_info_arr[] =
    {
...
esp_err_t ssd1306_init(ssd1306_t *dev, uint8_t width, uint8_t height, ssd1306_mode_t mode)
{
    ...
    for (size_t i = 0; i < sizeof(init_sequence); i++) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to initialize device (handle=%p, addr=0x%02X). err: %s",
                    (void*)dev->handle,
                    dev->address,
                    esp_err_to_name(ret));
            return ret;
    ...
    ESP_LOGI(TAG, "Device (handle=%p, addr=0x%02X) initialized  successfully",
            (void*)dev->handle,
            dev->address);
    return ESP_OK;
}
...
esp_err_t ssd1306_set_mode(ssd1306_t *dev, ssd1306_mode_t mode)
{
    ...
    dev->mode = mode;
    ESP_LOGI(TAG, "Device (handle=%p, addr=0x%02X) memory mode changed to 0x%02X",
            (void*)dev->handle,
            dev->address,
            (uint8_t)mode);
    return ESP_OK;
...
esp_err_t ssd1306_draw_char(ssd1306_t *dev, char c)
{
    ...
    return ssd1306_write_data(dev, bitmap, dev->font->width);
}
